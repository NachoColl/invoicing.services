...
## <span style="color:green">paypal/invoice/add</span>

```javascript
var paypalIPNMessage = "mc_gross=23.37&payer_id...";
$.ajax({
    method: 'POST',
    url: 'https://api.invoicing.services/paypal/invoice/add',
    data: paypalIPNMessage,
    ...
  });
```
...
InvoicingAPI API = new InvoicingAPI(MY_API_KEY);              
AddInvoiceResponse response = API.AddPaypalIPNMessageInvoice("mc_gross=23.37&payer_id...");

```

> The above command returns the same results as the [invoice/add](#invoice-add) method.

Use this method to create an Invoice from a PayPal IPN message.

<aside class="warning">
This method do not use JSON for parameters.
</aside>

### HTTP Request

`POST https://api.invoicing.services/paypal/invoice/add`

### Parameters

Just include the PayPal IPN message.

<aside class="info">
Please note that PayPal IPN messages do not always include all the information you may require.
<br/>We set some custom parameters you can add to initial IPN message.
</aside>

Custom Parameter | Type | Description 
--------- | -------  |  ----------- 
item_price[x] | decimal | Add this parameter to display unit prices for your invoice items. For example: item_price1=19.00


If you need to use a custom parameter, just append it to the original message: `mc_gross=23.37&payer_id...ipn_track_id=79e3ddb1dd4b1&item_price1=19.00`.
